...
st.caption("Analisis de IOC VirusTotal & AbuseIP")
...
def get_whois_info(target):
    ...
    try:
        ...
        if w.org: info.append(f"Organizacion: {w.org}")
        return "\n".join(info) if info else "WHOIS: Sin detalles publicos.", c_code
...
def get_status_label(verdict: str) -> str:
    labels = {
        "MALICIOSO": "[!]", 
        "SOSPECHOSO": "[?]", 
        "BAJO RIESGO": "[+]", 
...
def build_internal_block(ioc, ioc_type, verd, vt_m, vt_t, vt_l, details, whois_text, found_vt):
    label = get_status_label(verd)
    # Correccion del error: inicializamos 'text' directamente
    text = f"ANALISIS INTERNO SOC - {label} {verd}\n"
    text += f"--------------------------------------------------\n\n"
...
def build_analysis_block(ioc, ioc_type, verd, vt_m, vt_t, vt_l, details, found_vt):
    label = get_status_label(verd)
    text = f"ANALISIS IOC - {ioc}\n"
    text += f"--------------------------------------------------\n\n"
    text += f"RESULTADO: {label} {verd}\n\n"
    text += build_context_block(ioc_type, vt_m, vt_t, details, found_vt)
    text += f"\nENLACES\n--------------------------------------------------\n- VT: {vt_l}\n"
    if 'ab_l' in details: text += f"- Abuse: {details['ab_l']}\n"
    text += "--------------------------------------------------\n\n"
...
def render_copy_box(title: str, text: str, unique_key: str):
    ...
    comp_html = f"""
    <textarea id="{unique_key}" readonly style="width: 100%; height: 400px; padding: 15px; background: #f4f4f4; color: #333; font-family: monospace; border-radius: 8px; border: 1px solid #ccc;">{escaped_text}</textarea>
    <button onclick="navigator.clipboard.writeText(document.getElementById('{unique_key}').value)" style="margin-top: 10px; background: #333; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; font-weight: bold;">Copiar {title}</button>
    """
...
if st.button("Iniciar Analisis", type="primary", use_container_width=True):
    ...
    with st.spinner("Analizando..."):
        for ioc in input_list:
            ...
            if t == "IP":
                ...
                list_ips.append({"Estado": get_status_label(verd), "IP": ioc, "Pais": pais, "ISP": isp, "Abuse": f"{ab_s}%", "VT": f"{vt_m}/{vt_t}" if found_vt else "No encontrado", "VirusTotal": vt_l, "AbuseIPDB": ab_l})

            elif t == "Hash":
                sig = v_attr.get("signature_info", {})
                firm = "Valida" if sig.get("verified") == "Valid" else ("No valida" if sig else "No")
                ...
                list_hashes.append({"Estado": get_status_label(verd), "Hash": ioc, "Nombre": details['FileName'], "Firmado": details['Firmado'], "VT": f"{vt_m}/{vt_t}" if found_vt else "No encontrado", "VirusTotal": vt_l})
                ...
                list_urls.append({"Estado": get_status_label(verd), "URL/Dominio": ioc, "Categoria": details['Category'], "IP Resuelta": details.get('Resolved_IP', 'N/A'), "VT": f"{vt_m}/{vt_t}" if found_vt else "No encontrado", "VirusTotal": vt_l, "AbuseIPDB": ab_l})
    ...
    st.divider()
    c1, c2 = st.columns(2)
    with c1: render_copy_box("Investigacion Interna", full_internal, "int_box")
    with c2: render_copy_box("Analisis de IOC", full_analysis, "ana_box")
